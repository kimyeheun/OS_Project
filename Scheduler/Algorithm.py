...
class FCFS:
    ...
    def Running(self):
        ...
        self.ready.sort(key=lambda x: x.AT)
        ...
        while not self.endSimulation() or not self.q.empty():  # 큐가 빌 때까지
            if core.corenum > self.q.qsize():  # 코어 수가 큐에 있는 것보다 많거나 같을 때
                ...
                for i in range(core.corenum):
                    now = self.q.get()  # 기존 FCFS 큐는 계속 있음. 여기에 있는 것을 꺼낼 때마다 코어에 배정해주는 것.
            ...
            for k in range(self.q.qsize()):
                ...
                allwait.sort(key=lambda x: x.AT)  # BT로 정렬하기
            ...
            for j in range(0, self.num_process):
                if self.P[j].AT == self.time:
                    ...
                    allwait.sort(key=lambda x: x.AT)
...
class RR:
    ...
    def Running(self):
        ...
        # 맨 처음에는 self.time인 프로세스를 넣어줌
        # 예외. 프로세스 수보다 코어 수가 많으면 에러 남
        if core.corenum > self.q.qsize():  # 코어 수가 큐에 있는 것보다
        ...
        while not self.endSimulation() or not self.q.empty():  # 큐가 빌 때까지
            ...
            for j in range(self.num_process):
                if (self.P[j].AT == self.time):
                    arrive.append(self.P[j])
                    '''
                    self.arr_flag = 1           # 도착한 프로세스 있다는 표시
                    allwait.append(self.P[j])   # 남은 애들 넣어준 allwait에 도착한 애도 넣어줌
                    allwait.sort(key=lambda x: x.BT)    # 다시 BT 기준 정렬
                    '''
...
def doHRRN(processNum, originalInfo, pCoreCnt, eCoreCnt):
    ...
    ShortestRemaining = HRRN(process, num_process)
    ShortestRemaining.Running()
    resultInfo = ShortestRemaining.display()
    return resultInfo
...
def doBOSS(processNum, originalInfo, pCoreCnt, eCoreCnt):
    ...
    ShortestRemaining = BOSS(process, num_process)
    ShortestRemaining.Running()
    resultInfo = ShortestRemaining.display()
    return resultInfo
